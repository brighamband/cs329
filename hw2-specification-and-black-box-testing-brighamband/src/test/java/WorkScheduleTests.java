 ...
 * @ensures there are openings ==> returnValue >= 0 and returnValue <= size(ws)
 *          - 1
 * @ensures ws[returnValue].Hour.requiredNumber >
 *          size(ws[returnValue].Hour.workingEmployees)
 */
...
public class WorkScheduleTests {
  ...
  void testAddWorkingPeriod() {
    ...
    // Test Partition 2 -- If required number is already high enough, new employees
    // aren't added
  ...
  void testNextIncomplete() {
    ...
    assertTrue(
        ws.readSchedule(nextIncomplete).requiredNumber >= ws.readSchedule(nextIncomplete).workingEmployees.length);
  }
}
